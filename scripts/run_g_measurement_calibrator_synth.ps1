...
$tcl = Join-Path $PSScriptRoot 'synth_g_measurement_calibrator.tcl'
$timingReport = Join-Path $projectRoot `
    'results\synth_g_measurement_calibrator\timing_summary.rpt'
...
try {
    & $vivado -mode batch -nolog -nojournal -source $tcl 2>&1 |
        Tee-Object -FilePath `
            (Join-Path $logDir 'synth_g_measurement_calibrator.log')
    if ($LASTEXITCODE -ne 0) {
        throw "Measurement calibrator synthesis failed with exit code $LASTEXITCODE"
    }
